...
namespace GameFramework.Asset
{
    public class SceneRequest : LoadRequest
    {
        ...
        internal static SceneRequest LoadInternal(string bundleName, string assetName, bool withAdditive, System.Action<Request> callback)
        {
            var path = $"{bundleName}/{assetName}";

            var request = Unused.Count > 0 ? Unused.Dequeue() : new SceneRequest();
            request.Reset();
            request.bundName = bundleName;
