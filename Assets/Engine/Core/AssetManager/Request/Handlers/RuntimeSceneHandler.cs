...
namespace GameFramework.Asset
{
    public struct RuntimeSceneHandler : ISceneHandler
    {
        ...
        {
            _dependencies = Dependencies.LoadAsync(request.bundName);
        }
        ...
        {
            return SceneManager.LoadSceneAsync(Path.GetFileNameWithoutExtension(request.path), LoadSceneMode.Additive);
        }
        ...
        public void WaitForCompletion(SceneRequest request)
        {
            ...
            SceneManager.LoadScene(Path.GetFileNameWithoutExtension(request.path), LoadSceneMode.Additive);
