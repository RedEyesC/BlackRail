...
namespace GameLogic
{
    internal sealed partial class MovementController
    {
        ...
        public string CurrentStateName => ShouldReturnToIdle() ? "Idle" : "Locomotion";
        ...
        public void SetMoveInput(Vector3 worldMoveDirection, float inputAmount)
        {
            ...
            desiredMoveDirection = rawMoveInputAmount > inputDeadZone ? NormalizePlanar(worldMoveDirection) : Vector3.zero;
            moveInputAmount = HasMoveInput ? GetEffectiveInputAmount(rawMoveInputAmount) : 0f;
        }
        ...
        {
            UpdateDesiredYaw();
            UpdateCodeDrivenMovement(deltaTime);
        ...
        {
            UpdateLocomotionBlend(deltaTime);
            UpdateAnimationPlaybackSpeed();
        }

        // Locomotion mixer.
        private void DampLocomotionValue(float deltaTime, float targetValue)
        {
        ...
        private void UpdateLocomotionBlend(float deltaTime)
        {
            float targetValue = GetTargetLocomotionValue();
            DampLocomotionValue(deltaTime, targetValue);
        ...
        private void EnsureLocomotionPlaying(float parameter)
        {
            ...
            AnimationPlayResult result = owner.TryPlayAnimation(_locomotionTransition, out AnimPlayableComponent.State state);
        ...
        {
            return !HasMoveInput && GetCurrentPlanarSpeed() <= locomotionExitSpeed;
        }
        ...
        private void UpdateAnimationPlaybackSpeed()
        {
            if (!syncAnimationPlaybackSpeed)
            {
                if (_locomotionState != null && _locomotionState.IsValid)
                {
                    _locomotionState.PlaybackSpeed = 1f;
                }
        ...
        private void ApplyAnimationPlaybackSpeed(AnimPlayableComponent.State state)
        {
            ...
            state.PlaybackSpeed = CalculateAnimationPlaybackSpeed(state);
        }
        ...
        private float CalculateAnimationPlaybackSpeed(AnimPlayableComponent.State state)
        {
            ...
            float animatedPlanarSpeed = GetStateAnimatedPlanarSpeed(state);
        ...
        // Movement targets and speed calculation.
        ...
        // Code-driven movement.
        private void UpdateCodeDrivenMovement(float deltaTime)
        {
            ...
            RotateToDesiredMoveDirection(deltaTime);
            ApplyPlanarMovement(deltaTime);
        }
        ...
        private float GetVelocityAcceleration(Vector3 targetVelocity)
        {
            ...
            if (!HasMoveInput || targetVelocity.sqrMagnitude <= 0.0001f)
            {
        ...
        private void RotateToDesiredMoveDirection(float deltaTime)
        {
            ...
            facingYaw = NormalizeYaw(facingYaw + yawStep);
            facingForward = YawToDirection(facingYaw);
            owner.SetDir(facingForward.x, facingForward.z);
        }

        private void ApplyPlanarMovement(float deltaTime)
        {
        ...
        // Movement directions derived from input.
