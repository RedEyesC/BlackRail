...
namespace GameFramework.Interface
{
    public class ClickComponent : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IMoveHandler, IPointerClickHandler
    {
        ...
        public static ClickComponent Get(Transform t)
        {
            ClickComponent listener = t.gameObject.GetComponent<ClickComponent>();
            if (listener == null)
                listener = t.gameObject.AddComponent<ClickComponent>();
            return listener;
        }
        ...
        {
            if (touchEnable && onTouchDown != null)
                onTouchDown(eventData.position.x, eventData.position.y);

            if (onLongPress != null)
                InvokeRepeating("InvokeLongPress", LongPressDelayTime, LongPressInvokeRepeatTime);
        ...
        {
            if (touchEnable && onTouchUp != null)
                onTouchUp(eventData.position.x, eventData.position.y);

            if (onLongPress != null)
                CancelInvoke("InvokeLongPress");
        ...
        {
            if (touchEnable && onMove != null)
                onMove(eventData.moveVector.x, eventData.moveVector.y);
        }
