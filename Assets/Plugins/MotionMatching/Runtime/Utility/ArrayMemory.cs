...
namespace MotionMatching
{
    internal struct ArrayMemory : IDisposable
    {
        ...
        {
            return new ArrayMemory() { requiredSize = 0, usedSize = -1 };
        }
        ...
        internal void Reserve<T>(int numElements)
            where T : struct
        {
            int elemSize = 0;
        ...
        internal NativeSlice<T> CreateSlice<T>(int length)
            where T : struct
        {
            if (usedSize < 0)
            {
            ...
            {
                throw new Exception(
                    $"Memory overflow : trying to create a slice of {sliceSize} bytes whereas there are only {availableSize} bytes available"
                );
            }
