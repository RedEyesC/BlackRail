...
namespace MotionMatching
{
    ...
    public partial struct MotionSynthesizer
    {
        ...
        {
            return new MotionSynthesizer() { isValid = false };
        }

        internal static MotionSynthesizer Create(
            BinaryReference binaryRef,
            AffineTransform worldRootTransform,
            float blendDuration,
            Allocator allocator
        )
        {
        ...
        public bool Update(float deltaTime)
        {
            ...
            //    if (prevSamplingTime.IsValid)
            //    {
            //        AffineTransform referenceTransform = Binary.GetTrajectoryTransform(prevSamplingTime);

            //        rootDeltaTransform = referenceTransform.inverseTimes(Binary.GetTrajectoryTransform(deltaSamplingTime));
            //    }

            //    rootTransform *= rootDeltaTransform;
            //    rootTransform.q = math.normalize(rootTransform.q);

            //    trajectory.Update(rootTransform, rootDeltaTransform, deltaTime);

            //    poseGenerator.Update(rootTransform, samplingTime, deltaTime);
            //}
