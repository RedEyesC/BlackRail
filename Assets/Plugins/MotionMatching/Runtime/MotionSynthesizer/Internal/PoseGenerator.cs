    ...
    internal struct PoseGenerator
    {
        struct ScalarTransition
        {
            ...
            public static ScalarTransition Create(float x0, float v0, float t1)
            {
                ...
                float a0 = (-8.0f * v0 * t1 - 20.0f * x0) / MathEx.squared(t1);
        ...
        struct AngularTransition
        {
            ...
            public static AngularTransition Create(
            ...
            )
            {
                ...
                var axis = MathEx.axisAngle(displacement, out angle);

                if (angle < 0.0f)
                {
                    angle = -angle;
                    axis = -axis;
                    velocity = MathEx.negate(velocity);
                }
                ...
                var x1 = 2.0f * math.atan(math.dot(velocity.value.xyz, axis) / velocity.value.w);
        ...
        struct TransformTransition
        {
            ...
            public static TransformTransition Create(
                AffineTransform deltaTransform,
                AffineTransform velocityTransform,
                float durationInSeconds,
                float deltaTime
            )
            {
                ...
                var linear = LinearTransition.Create(deltaTransform.t, velocityTransform.t, durationInSeconds, inverseDeltaTime);

                var angular = AngularTransition.Create(deltaTransform.q, velocityTransform.q, durationInSeconds, inverseDeltaTime);

                return new TransformTransition { linear = linear, angular = angular };
            }
            ...
            {
                return new AffineTransform(linear.Evaluate(), angular.Evaluate());
            }
        }
        ...
        public void Update(AffineTransform worldRootTransform, SamplingTime samplingTime, float deltaTime)
        {
            ...
            for (int i = 0; i < numJoints; ++i)
            {
                ...
                currentPose.transforms[i] = targetPose.Ref.transforms[i] * deltaTransform;
            }
        ...
        void TriggerTransition(ref TransformBuffer targetPose, float deltaTime)
        {
            if (triggerTransition)
            {
                ...
                for (int i = 0; i < numJoints; ++i)
                {
                    ...
                    {
                        previousTransform.q = MathEx.negate(previousTransform.q);
                    }

                    if (math.dot(targetTransform.q, currentTransform.q) < 0.0f)
                    {
                        targetTransform.q = MathEx.negate(targetTransform.q);
                    }

                    var deltaTransform = targetTransform.inverseTimes(currentTransform);

                    var velocityTransform = targetTransform.inverseTimes(previousTransform);

                    transitions[i] = TransformTransition.Create(deltaTransform, velocityTransform, blendDuration, deltaTime);
                }
