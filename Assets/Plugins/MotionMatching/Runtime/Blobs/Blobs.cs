...
{
    internal unsafe struct BlobAllocator : IDisposable
    {
        byte* m_RootPtr;
        byte* m_Ptr;
        ...
        public void Allocate<T>(int length, ref BlobArray<T> ptr)
            where T : struct
        {
        ...
        public void Allocate<T>(ref BlobPtr<T> ptr)
            where T : struct
        {
    ...
    [NativeContainer]
    internal unsafe struct BlobAssetReference<T>
        where T : struct
    {
        ...
        public ref T Value
        {
            get { return ref UnsafeUtility.AsRef<T>(m_Ptr); }
        }

        public static unsafe BlobAssetReference<T> Create(byte[] data)
        {
    ...
    internal unsafe struct BlobPtr<T>
        where T : struct
    {
    ...
    internal unsafe struct BlobArray<T>
        where T : struct
    {
        internal int m_OffsetPtr;
