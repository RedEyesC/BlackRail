...
namespace MotionMatching
{
    internal static class BlobFile
    {
        ...
        public static unsafe void WriteBlobAsset<T>(BlobAllocator blobAllocator, ref T asset, string fileName)
            where T : struct
        {
        ...
        public static unsafe int ReadBlobAssetVersion(string filename)
        {
            if (Exists(filename))
            {
        ...
        public static unsafe BlobAssetReference<T> ReadBlobAsset<T>(string fileName)
            where T : struct
        {
            if (Exists(fileName))
            {
                return BlobAssetReference<T>.Create(File.ReadAllBytes(fileName));
            }
