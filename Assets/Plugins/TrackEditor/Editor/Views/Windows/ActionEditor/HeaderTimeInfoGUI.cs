...
namespace TrackEditor
{
    ...
    {
        public void DrawHeaderTimeInfoGUI()
        {
            if (asset == null)
                return;
            ...
            GUI.BeginGroup(topMiddleRect);
            {
                ...
                {
                    var label = doFrames ? (player.CurrentTime * Prefs.frameRate).ToString("0") : player.CurrentTime.ToString("0.00");
                    ...
                    GUI.backgroundColor = EditorGUIUtility.isProSkin ? Color.black.WithAlpha(0.4f) : Color.black.WithAlpha(0.7f);
                    GUI.color = player.GetScriberColor();
